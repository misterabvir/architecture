...
public class DeviceCacheRepository(DeviceRepository decorated, IDistributedCache cache) : IDeviceRepository
{
    ...
    {
        return await cache.GetOrCreate($"cpu-{id}", () => decorated.GetCpuById(id, cancellationToken), cancellationToken);
    }

    public async Task<List<Cpu>> GetCpus(CancellationToken cancellationToken = default)
    {
        return (await cache.GetOrCreate("cpus", () => decorated.GetCpus(cancellationToken)!, cancellationToken))!;
    }

    public async Task<Ip?> GetIpById(Guid id, CancellationToken cancellationToken = default)
    {
        return await cache.GetOrCreate($"ip-{id}", () => decorated.GetIpById(id, cancellationToken), cancellationToken);
    }

    public async Task<List<Ip>> GetIps(CancellationToken cancellationToken = default)
    {
        return (await cache.GetOrCreate("ips", () => decorated.GetIps(cancellationToken)!, cancellationToken))!;
    }    

    public async Task<Os?> GetOsById(Guid id, CancellationToken cancellationToken = default)
    {
        return await cache.GetOrCreate($"os-{id}", () => decorated.GetOsById(id, cancellationToken), cancellationToken);
    }

    public async Task<List<Os>> GetOss(CancellationToken cancellationToken = default)
    {
        return (await cache.GetOrCreate("oss", () => decorated.GetOss(cancellationToken)!, cancellationToken))!;
    }

    public async Task<Ram?> GetRamById(Guid id, CancellationToken cancellationToken = default)
    {
        return await cache.GetOrCreate($"ram-{id}", () => decorated.GetRamById(id, cancellationToken), cancellationToken);
    }

    public async Task<List<Ram>> GetRams(CancellationToken cancellationToken = default)
    {
        return (await cache.GetOrCreate("rams", () => decorated.GetRams(cancellationToken)!, cancellationToken))!;
    }

    public async Task<Rom?> GetRomById(Guid id, CancellationToken cancellationToken = default)
    {
        return await cache.GetOrCreate($"rom-{id}", () => decorated.GetRomById(id, cancellationToken), cancellationToken);
    }

    public async Task<List<Rom>> GetRoms(CancellationToken cancellationToken = default)
    {
        return (await cache.GetOrCreate("roms", () => decorated.GetRoms(cancellationToken)!, cancellationToken))!;
    }  
